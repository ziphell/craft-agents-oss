...
101. 给一条补丁加头注释 `/* @target .pay-btn */` → `prototype-status` 的补丁清单与详情页都显示这个 selector；没写的显示 `—`（不是猜一个）
102. 让 `prototype-apply` 遇到一个页面上不存在的 selector（`@target .typo`）→ 输出**点名** `matched nothing`，面板的 toast 同一句；改对再 apply → 不再点名
...
106. 用外部编辑器保存一条补丁 → overlay 窗口 1 秒内**自己**重放；scratch 页窗口自动刷新。关掉详情页的「改动自动重放」→ 再存一次，窗口不动
107. 连续保存三个文件（`patches/`、`assets/`、页文档各一个）→ 窗口只重放**一次**；两次重放之间，JS 补丁的副作用不叠加
108. 一条补丁没写 `@target` → apply 输出点名"no @target，无法检查"，而不是当作干净通过
109. 只有 overlay 页的原型上跑 `prototype-commit` → 该页补丁折成 `patches/<页名>/Z-001-upper.css` 与 `Z-002-upper.js`，被折的补丁文件**被删**；`prototype-status` 里补丁变成这两条，且 `@target` 与 `@requirement` 标记都还在（锚点与需求线因此不断）
110. 折入后 reload 那个 overlay 页 → 效果与折入前**一致**（Z 排最后，覆盖它折入的那些）
111. `prototype-commit` 在 scratch 页上跑 → CSS 进 `assets/<页名>/committed.css`、JS 升格进 `assets/<页名>/committed.js`，页文档里被插入 `<link>` 与 `<script src>` **各一次**，补丁文件被删；打开页面效果不变
112. 立刻再跑一次 `prototype-commit` → 报"没有可收敛的补丁"，一个字节都不写
113. 把一个 scratch 页的文档删掉（页表里仍声明它）→ `prototype-commit` 报 **the page document is missing**，并且**不删**那条补丁
114. `prototype-commit --page cart` → 只有 cart 自己的补丁被折；共享补丁与别的页的补丁原地不动
115. 折入一个 scratch 页的补丁之后 → 该页的锚点记录被清掉（元素已在我们自己的文件里，没有"页面漂移"可言）；同一个原型里 overlay 页的锚点记录**保留**
...
- **进入选择模式的条件从"有原型"改成"有会话"**（工具栏新增 `hasSession`）。没有会话绑定的窗口仍然不给选择器——它不知道该把结果给谁。
- **高亮框下方多一条工具条**：注入脚本里新增 `bar` 与一个按钮，且**只在有会话绑定时构建**——做不了事的按钮比没有按钮更糟。框上方仍是原有的选择器标签（stable selector）。
- **点击后走既有出口**：`PickedElement.intent = 'add-to-conversation'` 随元素回到主进程 → 广播 `{ kind: 'add-to-conversation' }` → 主窗口把它写进该会话的输入框（`onInputChange` + `craft:restore-input` 两步，与原型那条完全同源）。

四个实现要点：

- **意图挂在元素上**（`intent` 字段），而不是新开一条结果通道：选择器的返回类型、能力协议、agent 的 `pick` 命令三处都不用改签名，`PICKER_STATE_KEY` 的形状也不变。
...
- **解绑会话后要重推工具栏状态**：`unbindSession` / `unbindAllForSession` 原本刻意不推（"owner 链没变"），但按钮的显隐依赖 `boundSessionId`，所以这条推理不再成立。

#### 12.7.1 元素以 chip 进输入框，且是追加

输入框的真值是**纯字符串**（`rich-text-input` 是原生 contenteditable，chip 只是这个字符串的渲染投影），所以一个"token"要活过一次字符串往返，就只能**是**字符串：元素以 `[element:<selector>|<text>]` 落进文本，两段各做 percent-encoding（选择器里本来就有 `[]`，而标记的正则读到第一个 `]` 为止）。

- **放哪**：`renderer/lib/element-mention.ts` —— 编码 / 解析 / 查找 / 展开四个纯函数，带单测。`findMentionMatches` 认得这个标记，但 `ComposerMentionType` 显式把 `element` 排在 `MentionItemType` **之外**：`@` 菜单永远产不出它，把它塞进菜单那个联合类型会误导读者。`rich-text-input` 的 badge 机制原样复用——同一套 `data-mention` 原子节点、同一套光标与选中处理——只多一个图标和解码后的标签。
- **谁看得见什么**：chip 标签取元素自己的文字，没有文字（图标、空容器）才退回选择器；hover 给选择器，两个元素文字相同时仍能分辨。
- **出去时展开**：标记**不进消息**。`FreeFormInput` 在 `submitMessage` 与草稿快照两处把它换成 `browserEdit.elementReference` 的可读引用——模型该读到句子，不是 payload。这也顺带省掉了 agent 侧再挂一个 resolver。
...
- **会话自己的绑定优先**：`会话绑定 ?? 项目的原型`。所以 `prototype-bind <slug>` 仍然是覆盖手段，`prototype-bind --clear` 之后回到项目级（不是"没有原型"）。

读点只有三处，改这三处就全线一致（**窗口**的地址栏与两个原型动作、agent 的 `prototype-*` 默认 slug、系统提示词里的 `<prototype_context>`）：`SessionManager.effectivePrototypeSlug` 一处，`getSessionPrototypeBinding` / agent 配置装配 / `getBoundPrototypeSlug` 三处调用它。渲染层不需要第二套逻辑——它手里的窗口信息（`toInfo().prototypeSlug`）本来就是主进程用同一个解析算出来的。
...
### 20.1 需求锚点：PRD 是文件，id 是机制

- **`prd.md`**（原型目录下）由 agent 写，形状固定：`## R-001 <一句话>` + 其下正文（为谁、今天怎样、什么必须成立）。
- **id 是全部机制**：补丁头写 `@requirement R-001`，页文档里写注释。两者共用一个提取器，因为"这个改动服务哪条需求"是同一个问题，不分文件类型。书写宽容（`r-1` / `R-1` / `R-001` 同一），`TBD` 之类一律忽略——不猜 id。
- **线程派生，且只在一处派生**（[coverage.ts](file:///c:/Users/Ryan/code/craft-agents-oss/packages/shared/src/prototypes/coverage.ts)）：`prototype-status` 与 `dist/dev-spec.md` 读同一份解析结果，因此不可能给出不同答案。
...
**已知缺口**：用户**手动**操作（不用 agent 工具）产生的变化会被变化检测抓到（有帧），但没有动作标签——标签只来自我们发出的 CDP 动作。要补齐需要在页面里注入监听器并回传，是独立一项。
...
**入口两处**：命令 `prototype-record import <path>`，与详情页 Frames 区块的按钮（文件选择器跑在主进程，路径不经过渲染层 → 本地与远程会话同构）。
...
`prototype-verify [slug]` 逐条跑，写 `dist/acceptance.md`（交付物之一，与 dev-spec 并列——它们的读者是同一个人、同一时刻）。
...
- **是列表，不是一个**：`prototype-commit` 会把多条补丁折进一个文件，并把每个来源的标记写进 provenance 注释里——只读第一个标记的话，其余锚点会在折入的**那一刻**变成"没有补丁声明的记录"。
...
- **不做自动 commit，也不做撤销**：它是不可逆的合并（用户自己的 git 才是"前后"的地方），只能是一个明确动作；第二次 commit 会说"没有可收敛的补丁"，而不是写一个空文件。
...
- 300ms 合并（多文件保存是一次意图）、**默认开**、可以在详情页关掉；这个开关存在 `~/.craft-agent/preferences.json` 里（是"这个应用怎么盯文件"，不是原型的事实）。默认开是因为"保存了却没反应"正是这个功能要消掉的东西；而关掉的理由同样是实际的——重放一个**活页面**会刷新它，把窗口里正在填的东西丢掉，这个决定属于知道那个窗口在干什么的人。
...
- **命令面**：`prototype-commit [slug] [--page <name>]`；`prototype-apply` 的输出增加三类点名（matched nothing / the page moved + 候选 / no @target，无法检查）；`<prototype_context>` 多一条规矩（每条补丁写 `@target`）与一个说明（commit 折什么、为什么不可逆）。

**未做**：

| 项 | 为什么 |
| --- | --- |
| js 的关键断言（S4 的后半句） | 断言属于 `prototype-verify` 的 `check:` 行（§20.7）；补丁这边只报"有没有抛错"，再多就是第二套校验语言 |
