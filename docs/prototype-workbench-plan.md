...
| `pages` | `--add name=url` 仍是 overlay 页；**不带 url 只在该页文件已存在时接受**（声明是为了顺序，不是为了造页——scratch 页要有文档才算存在，写文档是 agent 的文件工具，§13.2 的"不预置空白"不变）；`--remove` 删的是**表里的一行** |
...
> **修订（实时采集整条撤掉，只剩 `sample-video`）**：`record <for>` 曾做成"看着这个标签页一段时间、留下画面变了的那几帧"的一次调用（更早是 `record start` / `record stop` 两步），**后来整条删掉**。理由不是实现难，而是**人和 agent 配合不起来**：它唯一不可替代的用法是"你操作、我看着"——录制期间页面要被人或 agent 驱动——而一次工具调用里没人能对另一头说"就现在，点它"；agent 自己驱动则等于在 recorder 里再实现一套输入原语。能用的替代都在：**要看"某次操作改了什么"，`screenshot` 就在动作之后**（同一次对话里）；**要论证一段过程，本来就该是别人录好的视频**，走 `sample-video`（§20.5）。随后删掉的：命令与它的帮助/示例、`BrowserPaneFns` 的 `startPrototypeFrames` / `stopPrototypeFrames`、`server-core` 的 `domain/record-prototype-frames.ts`、浏览器面板的 `startFrameCapture` / `stopFrameCapture`（内存帧、变化检测、上限、动作钩子）与 `browser-cdp.ts` 里那个"把 CDP 流量翻译成动作"的 `onAction` 钩子、能力白名单里的两条、以及配套的测试桩与文档。这一节的**两个采样器、动作标签、坐标、上限**这些判断仍然成立，只是它们现在只由 §20.5 的导入路径兑现（那一条产的帧同样带坐标与 `index.md`）。
>
...
- 300ms 合并（多文件保存是一次意图）、**默认开**、可以在详情页页头的 `…` 菜单里关掉；这个开关存在 `~/.craft-agent/preferences.json` 里（是"这个应用怎么盯文件"，不是原型的事实）。默认开是因为"保存了却没反应"正是这个功能要消掉的东西；而关掉的理由同样是实际的——重放一个**活页面**会刷新它，把窗口里正在填的东西丢掉，这个决定属于知道那个窗口在干什么的人。
