...
- **详情页只剩三个动作**：看（预览）、改（在对话里改）、交（导出给开发）。这是产品的全部，也是这一轮收敛的终点。
...
72. 三种拒绝路径都要试：对 **scratch** 说「把它指到某个地址」→ 拒绝（"its page is its own base.html"）；地址写成 `app.example.com/x`（缺 scheme）→ 拒绝并提示补上 scheme；在**没绑定**的会话里跑 → 拒绝并提示 `prototype-bind`（这条命令唯一的位参数是地址，所以它不认位置上的 slug）
73. 改完再 `prototype-export` → `dev-spec.md` 第一行与预览载体上的地址都换成新的；**旧窗口里的页面不会自己变**——`prototype-open` 一下才挪过去（这条正是命令里那句话的验收）
...
所以：`prototype-target <url>` 把**当前会话绑定的**那个 overlay 指到另一个地址。它只强制类型自身的规矩（scratch 拒绝、地址必须是浏览器能打开的 http/https），其余一概不拦。
...
- **overlay 的 `targetUrl`：创建时必填，之后可用命令改** —— 两件事不矛盾，管的是不同的东西：**必填**是因为创建时你正对着那个页面，不填则这个原型连"它是什么"都答不上来（§13.2）；**可改**是因为同一个页面存在于多套环境，那是事实不是规则（§13.2.1）。改走 `prototype-target <url>`，代价（旧窗口、旧选择器）由命令输出说清。**没有 UI 入口**：与「编辑只走对话」同一条理由（§12.4）——换环境是罕见且带风险的动作，它不该在详情页占一个常驻按钮
- **多目标页** —— overlay 只记一个 `targetUrl`，所以"这个原型覆盖哪几页"没有地方登记；换页就是换地址（`prototype-target`）。同一个窗口导航到别的页面时 patch 照样重放（注入按 document 生效），只是那个页面不在记录里
