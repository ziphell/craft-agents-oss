...
| 21.6 窗口里的直接编辑（新增） | 无。窗口的两种手势（准星选中 → 交给对话；地址栏输入 → 打开）都只把元素**指出来**，改页面只能由 agent 写补丁 | 窗口工具栏多一个「编辑」按钮，与准星**互斥**（两个 overlay 都要吃页面的点击）。页面侧（`browser-cdp.ts` 的 `buildEditorInjectScript`）是一个**草稿编辑器**：拖矩形框选（矩形内**最深的非 inline 元素**，一次可多选；点击 = 零面积矩形，所以点选和框选是同一条规则）、**B / I**（读页面 computed style 决定开关，多选即"全都要"）、双击元素改文字（挂 `contenteditable`，Enter/blur 提交、Esc 还原，提交的是**整块元素**的文字），浮动条是 `B · I · 撤销 · 保存 {n} · 放弃`，**只要还有未保存的编辑就一直在**（没有选中时停在右下角——保存住在条子上，让一个编辑没人按得到保存比条子悬空更糟）。**保存前不写任何文件**：草稿在页面里——样式进我们自己的一层 `<style>`（由草稿**按顺序生成**，所以"撤销"= 少一条后重画），文字就是 contenteditable 已经改掉的那份。**离开是一个提问而不是一个动作**：有未保存时，条子切成 `保存 {n} · 放弃` 两个按钮、模式不结束（此时页面上其它操作都不接受，Escape 再按一次 = 放弃并退出），选了才走。因此有两条出口：`askEditorToLeave`（工具栏按钮走的软请求：页面可以拒绝并在自己条子上问）与 `teardownEditor`（硬拆除：关标签页、换页重新 arm、以及收到 `cancelled` 之后的收尾——那些场合已经没人可问）。点保存 → 工具栏动作 `edit-requested`（载荷是这一批编辑）→ 渲染层用**标签页的** `prototype` / `prototypePage` 解析出 slug 与页 → RPC `prototypes:edit` → `writePrototypeEdits()` 写**一条**差量。互斥与"跟随前台标签页"由主进程的 `armEditor` / `disarmEditor` / `runEditorLoop` 负责，与 picker 同形；条上的文案由工具栏渲染层随 `startEditing` 传入（页面没有 i18n，picker 的 `addLabel` 是同一个道理）。**已知边界**：切标签页 = 硬拆除（草稿跟着没了），因为模式按设计跟随前台标签页，而草稿的目标元素在原来那一页上 | 直接编辑的价值全在"它被记下来了"：不落盘则刷新即失、别的窗口看不见、交付里也没有——所以落点只能是补丁，而补丁一旦是落点，`@target`、锚点、漂移、折叠、`dist/` 全都自动接上（§21 那一整套本来就在等一个写入者）。**"一次编辑一条补丁"被否掉的原因有两个**：一是人一次会话要改好几处，粒度太碎会让撤销变成"删文件的历史"；二是**写盘就会触发重放**，而重放会让 scratch 页重渲染——于是会话自己的状态（选区、草稿）每点一次就被冲掉，连改几个元素都做不到。所以写入推迟到"保存"这一个时刻：撤销在草稿里是免费的（重新生成那层样式 / 把元素文字放回去），会话期间页面不被打断。**代价是保存前页面显示的是草稿**——没有任何补丁声明过的值，正是这个仓库认定最坏的那种第二份描述；它由模式与浮动条明确承担：未保存计数、保存/放弃两个出口、离开前未保存的一律还原（`cleanup()` 里也走同一条 `discard()`）。文字只做**整块替换**：段内富文本要把元素的标记存进补丁，那就是 §14.1 否掉的"执行后序列化 DOM" |
