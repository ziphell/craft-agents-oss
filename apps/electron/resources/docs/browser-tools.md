...
- **A probe, not persistence.** What `evaluate` runs is not registered for any future document, so a reload restores the page and the change is gone. A change that has to survive a reload — **any overlay's real change** — is a patch file (`patches/<page>/<writer>-{nnn}-{name}.js`), which `prototype_tool apply` injects *and* registers (see `apply` in `~/.craft-agent/docs/prototypes.md`). Use `evaluate --file` to answer a question about the page, not to leave something behind.
...
- What it does to what was injected is the point of the two carriers: a live page keeps the patches `prototype_tool apply` registered (init scripts, which is what registering them buys) and drops anything `evaluate` ran, which was never registered.

### Prototypes — `prototype_tool`

Everything about prototypes has its own guide and its own tool, because a prototype is a whole workflow rather than one command. `prototype_tool` acts on a prototype's **files** (its folder under `{workspace}/prototypes/{slug}/`) and drives this same window to show the flow — while everything on this page is the window's own surface: refs, snapshots, input, console, network, tabs. Both tools go through one runtime, and they take bare command names, so the ones on this page and the ones on that tool's are told apart by which tool you call (`browser_tool open` opens the window; `prototype_tool open` opens a prototype).

**`~/.craft-agent/docs/prototypes.md`** — read it before your first `prototype_tool` command: what a prototype is (a flow of pages, each `scratch` or `overlay`), its files and directory layout, who owns which artifact, `PRD.md` and the material beside it / `research/` / `reviews/`, the `patches/` layer, folding the change layer (an option of copying, never a command), verification, contracts and mocks, the deliverables, and the complete command reference.

The short version: a prototype is a folder under `{workspace}/prototypes/{slug}/`. In `prototype_tool`, `list` shows what exists, `create <name>` makes one (with no pages — write `cart.html` and that *is* the first page), `open` opens it in a tab of its own, `apply` replays its patches, `status` reports what is there, and `export` builds the deliverable.
