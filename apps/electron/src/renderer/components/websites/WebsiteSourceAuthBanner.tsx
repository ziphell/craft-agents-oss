...
/**
 * Trusted-chrome reconnect banner: when a source this website holds grants on
 ...
 * (`deriveConnectionStatus`), never from anything the sandboxed website says,
 * so a website cannot trigger, spoof, or spam an auth prompt. Rows clear
 * automatically when reauthentication lands (`sources:changed` → props).
 */

interface WebsiteSourceAuthBannerProps {
  workspaceId: string
...
/** Sources this website's grants depend on (stale grants included — the website will re-request them). */
function grantedSourceSlugs(website: LoadedWebsite): Set<string> {
  const slugs = new Set<string>()
  for (const grant of website.config.grants ?? []) {
...
export function WebsiteSourceAuthBanner({ workspaceId, website, sources, className }: WebsiteSourceAuthBannerProps) {
  ...
  const needing = React.useMemo(() => {
    const granted = grantedSourceSlugs(website)
  ...
  const handleOAuth = React.useCallback(async (source: LoadedSource) => {
    ...
    try {
      ...
      if (!result.success) {
        toast.error(t('toast.websiteSourceReconnectFailed', { name: source.config.name }), {
  ...
  const handleReconnect = React.useCallback((source: LoadedSource) => {
    ...
    if (flavor === 'oauth') {
      ...
      navigate(routes.action.newSession({
        input: t('websites.auth.fixWithAgentPrompt', { name: source.config.name }),
      }))
    }
        ...
        return (
          <div key={source.config.slug} className="flex items-center gap-2.5">
            ...
            <span className="min-w-0 flex-1 text-xs text-foreground/70">
              {t('websites.auth.sourceNeedsReconnect', { name: source.config.name })}
            </span>
            <Button
              variant="outline"
              size="sm"
            ...
            >
              ...
              {busySlug === source.config.slug
                ? t('websites.auth.reconnecting')
                : flavor === 'agent'
                  ? t('websites.auth.fixWithAgent')
                  : t('websites.auth.reconnect')}
            </Button>
          </div>
        )
      })}
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('websites.auth.credentialTitle', { name: source?.config.name ?? '' })}
          </DialogTitle>
          <DialogDescription>{t('websites.auth.credentialDescription')}</DialogDescription>
        </DialogHeader>

        <div className="flex flex-col gap-1.5">
          <span className="text-sm">
            {t('websites.auth.credentialLabel')}
        ...
        <DialogFooter>
          ...
          <Button disabled={saving || value.trim().length === 0} onClick={() => void handleSave()}>
            ...
            {t('websites.auth.credentialSave')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
