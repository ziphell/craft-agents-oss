...
import type { LoadedWebsite, WebsiteActionGrant } from '@craft-agent/shared/websites/types'
 ...
 * CRAFT_FEATURE_WEBSITES_SHARING flag); `sharingEnabled` only controls what the
 * dialog offers. Unpublish is always offered for a published website so a
 * disabled flag can never strand a public copy (design §12).
 */

/**
 * Client-side mirror of the Worker's PASSWORD_MIN_LENGTH (workers/websites/
 ...
 */
const WEBSITE_PASSWORD_MIN_LENGTH = 8

/**
 * WebsiteShareError carries its machine code as a "WEBSITE_X: " message prefix so
...
export function useWebsiteShareCapabilities(): { sharingEnabled: boolean; loaded: boolean } {
  ...
  React.useEffect(() => {
    ...
    if (!capabilitiesPromise) {
      capabilitiesPromise = window.electronAPI.getWebsiteShareCapabilities().catch(() => {
...
export function ShareWebsiteDialog({
  ...
  // locally either, so they don't require it (mirrors buildWebsiteShareBundle).
  const hasUsableActionGrants = allGrants.some(
    g => g.action.kind !== 'script' && isWebsiteGrantUsable(g, config.contentDigest, Date.now()),
  )
  ...
  const passwordTooShort = password.length > 0 && password.length < WEBSITE_PASSWORD_MIN_LENGTH
  ...
  React.useEffect(() => {
    ...
    let cancelled = false
    window.electronAPI
      .getWebsiteShareDataScan(workspaceId, config.slug)
  ...
  const runPublish = React.useCallback(async () => {
    setBusy('publish')
    try {
      await window.electronAPI.publishWebsite(workspaceId, config.slug, {
        includeData,
        // Password only applies at create time; changes go through setWebsitePublicationPassword.
        ...(share ? {} : password ? { password } : {}),
        ...(hasUsableActionGrants ? { viewOnlyAcknowledged: true } : {}),
      })
      toast.success(t('toast.websitePublished'))
      setPassword('')
    } catch (err) {
      toast.error(t('toast.websitePublishFailed'), {
  ...
  const runSetPassword = React.useCallback(async (value: string | null) => {
    setBusy('password')
    try {
      await window.electronAPI.setWebsitePublicationPassword(workspaceId, config.slug, value)
      toast.success(t('toast.websitePasswordUpdated'))
      setPassword('')
    } catch (err) {
      toast.error(t('toast.websitePasswordUpdateFailed'), {
        description: displayShareError(err),
      })
    } finally {
      setBusy(null)
  ...
  const runUnpublish = React.useCallback(async () => {
    setBusy('unpublish')
    try {
      const result = await window.electronAPI.unpublishWebsite(workspaceId, config.slug)
      if (result.warning === 'remote-copy-may-remain') {
        toast.warning(t('toast.websiteUnpublished'), { description: t('toast.websitePublicCopyMayRemain') })
      } else {
        toast.success(t('toast.websiteUnpublished'))
      }
      onOpenChange(false)
    } catch (err) {
      toast.error(t('toast.websiteUnpublishFailed'), {
        description: displayShareError(err),
      })
    } finally {
      setBusy(null)
  ...
  const copyLink = React.useCallback(async () => {
    ...
    try {
      ...
      setCopied(true)
      toast.success(t('websites.share.linkCopied'))
  ...
  return (
    <Dialog open={open} onOpenChange={next => { if (!busy) onOpenChange(next) }}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{t('websites.share.title')}</DialogTitle>
          {!share && <DialogDescription>{t('websites.share.publishDescription')}</DialogDescription>}
        </DialogHeader>
        ...
        {!share ? (
          ...
          <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="flex items-start justify-between gap-3">
                  <span className="flex flex-col gap-0.5">
                    <span className="text-sm">{t('websites.share.includeData')}</span>
                    <span className="text-xs text-foreground/50">{t('websites.share.includeDataHint')}</span>
                  </span>
                  <Switch checked={includeData} onCheckedChange={setIncludeData} disabled={busy !== null} />
                </label>
                {secretCandidates.length > 0 && (
                  <span className="text-xs text-amber-500">
                    {t('websites.share.dataSecretWarning', { keys: secretKeysPreview })}
                  </span>
                )}
              </div>
            )}
            ...
            <div className="flex flex-col gap-1.5">
              <span className="text-sm">{t('websites.share.passwordLabel')}</span>
              <Input
                type="password"
                autoComplete="new-password"
                value={password}
                onChange={e => setPassword(e.target.value)}
                placeholder={t('websites.share.passwordPlaceholder')}
                disabled={busy !== null}
              />
              <span className={`text-xs ${passwordTooShort ? 'text-amber-500' : 'text-foreground/50'}`}>
                {passwordTooShort
                  ? t('websites.share.passwordTooShort', { min: WEBSITE_PASSWORD_MIN_LENGTH })
                  : t('websites.share.passwordHint', { min: WEBSITE_PASSWORD_MIN_LENGTH })}
              </span>
            </div>
            ...
            {scriptGrants.length > 0 ? (
              ...
              <label className="flex items-start justify-between gap-3 rounded-md border border-amber-500/30 bg-amber-500/5 p-3">
                <span className="flex flex-col gap-0.5">
                  <span className="text-sm">{t('websites.share.actionsAckLabel')}</span>
                  <span className="text-xs text-foreground/60">{t('websites.share.actionsAck')}</span>
                </span>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <span className="text-sm">{t('websites.share.linkLabel')}</span>
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={copyLink} className="shrink-0">
                  ...
                  {t('websites.share.copyLink')}
                </Button>
              </div>
              <span className="inline-flex items-center gap-1.5 text-xs text-foreground/50">
                {share.passwordProtected
                  ? <><Lock className="h-3 w-3" /> {t('websites.share.passwordProtected')}</>
                  : <><Globe2 className="h-3 w-3" /> {t('websites.share.noPassword')}</>}
              </span>
            </div>

            {share.lastPublishError && (
              <Info_Alert variant="error" inline icon={<AlertTriangle className="h-4 w-4" />}>
                <Info_Alert.Title>{t('websites.share.lastError')}</Info_Alert.Title>
              ...
              <>
                ...
                <div className="flex items-center justify-between gap-3 rounded-md border border-border/60 p-3">
                  <span className="text-xs text-foreground/60">
                    {contentDrifted ? t('websites.share.contentUpdateAvailable') : t('websites.share.contentUpToDate')}
                  </span>
                  <Button
                ...
                <div className="flex flex-col gap-2 rounded-md border border-border/60 p-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      value={password}
                      onChange={e => setPassword(e.target.value)}
                      placeholder={share.passwordProtected ? t('websites.share.changePassword') : t('websites.share.passwordPlaceholder')}
                      disabled={busy !== null}
                    />
                    <Button
                      variant="outline"
                  ...
                  {share.passwordProtected && (
                    ...
                    >
                      {t('websites.share.removePassword')}
                    </Button>
                  )}
                </div>
              </>
            ) : (
              <span className="text-xs text-foreground/50">{t('websites.share.disabledNote')}</span>
            )}
          </div>
        )}
 ...
 * server refuses to publish a website holding ANY script grant (even a stale
    ...
    <div className="flex flex-col gap-2 rounded-md border border-red-500/40 bg-red-500/[0.06] p-3">
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 text-sm">
          <div className="font-medium">{t('websites.share.scriptBlockedTitle')}</div>
          <div className="mt-0.5 text-xs text-foreground/60">{t('websites.share.scriptBlocked')}</div>
        </div>
      </div>
