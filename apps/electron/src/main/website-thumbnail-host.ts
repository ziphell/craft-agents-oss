/**
 * Pure helpers for website thumbnail capture — NO electron imports, so this is
 * unit-testable under bun. The Electron capture itself lives in
 * website-thumbnailer.ts and consumes these.
 *
 * The capture renders the website exactly as WebsiteFrame does: the website HTML is the
 * srcDoc of an opaque sandboxed iframe (never `allow-same-origin`), and the
 * trusted host posts the `craft-websites/v1` init message so data-driven websites
 * paint with their snapshot. No action bridge — a poster never executes source
 * actions.
 */

import type { WebsiteDataSnapshot, WebsiteKind } from '@craft-agent/shared/websites/types'
...
/** Same sandbox rule as WebsiteFrame: static = inert, interactive/live = scripts+forms, never same-origin. */
export function sandboxForKind(kind: WebsiteKind): string {
 ...
 * the website content in a sandboxed iframe and delivers the data snapshot once
 * via the same bridge WebsiteFrame uses. Returns a full HTML string; the caller
...
export function buildThumbnailHostHtml(input: {
  ...
  const website = JSON.stringify({ slug: input.slug, kind: input.kind })
...
<script>
  ...
  function deliver() {
    try {
      frame.contentWindow.postMessage(
        { protocol: 'craft-websites/v1', type: 'init', payload: { website: PAGE, nonce: 'preview', snapshot: SNAPSHOT } },
        '*'
      );
  ...
  window.addEventListener('message', function (event) {
    var m = event.data;
    if (m && m.protocol === 'craft-websites/v1' && m.type === 'ready') deliver();
  });
</script>
</body>
</html>`
