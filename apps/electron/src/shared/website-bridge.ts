/**
 * Website bridge protocol — the postMessage contract between a rendered Website
 * (opaque sandboxed iframe, srcDoc) and the trusted host (WebsiteFrame).
 *
 ...
 *   1. `event.source` is exactly the frame's contentWindow (WebsiteFrame's job),
 *   2. `event.origin === 'null'` (WebsiteFrame's job),
 *   3. the message parses against this module's strict schema with bounded
 *      sizes (this module's job),
 *   4. privileged messages echo the render-lease nonce (WebsiteFrame's job) —
 *      and the server-side WebsiteActionBroker re-validates the lease, nonce,
 *      replay cache, and grant independently.
 *
 * Message flow (all envelopes carry `protocol: 'craft-websites/v1'`):
 *
 *   host → website
 *     init          { website: {slug, kind}, nonce, snapshot, grants }  on load / 'ready'
 *     data          { snapshot }                              live websites only
 *     action-result { result: WebsiteActionResult }              response to 'action'
 *     grants        { grants: WebsiteGrantSummary[] }            reply to 'grant-request' + on grant changes
 *
 *   website → host
 ...
 *
 * Descriptors come in three kinds (see WebsiteActionDescriptor): `api`, `mcp`, and
 ...
 * never in the website message) and it always counts as mutating.
 *
...
import { hasPathTraversal } from '@craft-agent/shared/websites/types'

export const WEBSITE_BRIDGE_PROTOCOL = 'craft-websites/v1'
...
const HTTP_METHODS: readonly WebsiteActionHttpMethod[] = ['GET', 'POST', 'PUT', 'PATCH', 'DELETE']
const SCRIPT_RUNTIMES = ['bun', 'node', 'python3'] as const

// ============================================================================
// Incoming (website → host)
// ============================================================================

/** One capability a website asks the user to approve. `key` is the website's own correlation id. */
export interface WebsiteGrantRequestEntry {
  key: string
  description?: string
  action: WebsiteActionDescriptor
}

export type WebsiteBridgeIncoming =
  | { type: 'ready' }
  | { type: 'action'; requestId: string; nonce: string; grantId: string; invocation: WebsiteActionInvocation }
  | { type: 'action-cancel'; requestId: string; nonce: string }
  | { type: 'open-url'; nonce: string; url: string }
  | { type: 'grant-request'; nonce: string; requests: WebsiteGrantRequestEntry[] }

/**
 * What a website learns about an approved grant: enough to invoke it (id) and to
 * match it to a need (action descriptor + expiry) — never digests or internals.
 */
export interface WebsiteGrantSummary {
  id: string
  action: WebsiteActionDescriptor
  expiresAt: number
}

export function toGrantSummary(grant: WebsiteActionGrant): WebsiteGrantSummary {
  return { id: grant.id, action: grant.action, expiresAt: grant.expiresAt }
}

/**
 * Reconcile the frame's grant list against the usable grants from the website
...
export function reconcileGrantSummaries(
  current: WebsiteGrantSummary[],
  fromConfig: WebsiteGrantSummary[],
...
export function grantIdsEqual(a: WebsiteGrantSummary[], b: WebsiteGrantSummary[]): boolean {
...
export function descriptorEquals(a: WebsiteActionDescriptor, b: WebsiteActionDescriptor): boolean {
...
function parseInvocation(value: unknown): WebsiteActionInvocation | null {
  if (!isPlainObject(value)) return null
  if (value.kind === 'api') {
    if (!HTTP_METHODS.includes(value.method as WebsiteActionHttpMethod)) return null
    ...
    return {
      kind: 'api',
      method: value.method as WebsiteActionHttpMethod,
      path: value.path,
...
/** Validate an untrusted grant descriptor (what a website may ASK for). */
function parseDescriptor(value: unknown): WebsiteActionDescriptor | null {
  if (!isPlainObject(value)) return null
  if (value.kind === 'api') {
...
/**
 * Validate a website-supplied script path: bounded, relative, no ".." escape.
...
function parseGrantRequestEntries(value: unknown): WebsiteGrantRequestEntry[] | null {
  if (!Array.isArray(value) || value.length === 0 || value.length > MAX_GRANT_REQUESTS) return null
  const entries: WebsiteGrantRequestEntry[] = []
 ...
 * the caller must drop such events silently (websites can post arbitrary junk).
 */
export function parseWebsiteBridgeMessage(data: unknown): WebsiteBridgeIncoming | null {
  if (!isPlainObject(data) || data.protocol !== WEBSITE_BRIDGE_PROTOCOL) return null
...
export function isMutatingInvocation(invocation: WebsiteActionInvocation): boolean {
  ...
  // requires a real click, so a website can never fire it from a timer or on load.
  if (invocation.kind === 'api') return invocation.method !== 'GET'
  return true
}

// ============================================================================
// Outgoing (host → website)
// ============================================================================

export function buildWebsiteInitMessage(
  website: { slug: string; kind: WebsiteKind },
  nonce: string,
  snapshot: WebsiteDataSnapshot | null,
  grants: WebsiteGrantSummary[] = [],
): Record<string, unknown> {
  return {
    protocol: WEBSITE_BRIDGE_PROTOCOL,
    type: 'init',
    payload: { website, nonce, snapshot, grants },
  }
}

/** Current usable grants, sent in reply to 'grant-request' and after approvals. */
export function buildWebsiteGrantsMessage(grants: WebsiteGrantSummary[]): Record<string, unknown> {
  return {
    protocol: WEBSITE_BRIDGE_PROTOCOL,
    type: 'grants',
    payload: { grants },
...
export function buildWebsiteDataMessage(snapshot: WebsiteDataSnapshot | null): Record<string, unknown> {
  return {
    protocol: WEBSITE_BRIDGE_PROTOCOL,
    type: 'data',
    payload: { snapshot },
...
export function buildWebsiteActionResultMessage(result: WebsiteActionResult): Record<string, unknown> {
  return {
    protocol: WEBSITE_BRIDGE_PROTOCOL,
    type: 'action-result',
    payload: { result },
...
export class WebsiteActionRateLimiter {
