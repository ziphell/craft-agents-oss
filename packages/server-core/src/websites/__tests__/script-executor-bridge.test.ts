/**
 * Websites script executor: builds a ScriptAction from the grant invocation,
...
import { createWebsitesScriptExecutor } from '../script-executor-bridge'
...
function makeExecutor(result: Partial<ScriptActionResult>) {
  ...
  const executor = createWebsitesScriptExecutor({
...
describe('createWebsitesScriptExecutor', () => {
  test('runs the pinned script and returns exit/stdout/stderr', async () => {
    ...
    const out = await executor(
      { websiteSlug: 'dash', script: 'websites/dash/run.sh', runtime: 'bun', args: ['--once'] },
      { signal },
    )
    expect(out).toEqual({ exitCode: 0, stdout: 'done', stderr: '' })
    expect(seen[0].action).toEqual({ type: 'script', script: 'websites/dash/run.sh', runtime: 'bun', args: ['--once'] })
  })

  test('never sets ScriptAction.page (must not clobber the refresh marker)', async () => {
    const { executor, seen } = makeExecutor({ exitCode: 0 })
    await executor({ websiteSlug: 'dash', script: 'websites/dash/run.sh' }, { signal })
    expect(seen[0].action.page).toBeUndefined()
  })

  test('injects CRAFT_ workspace + website env for the triggering website', async () => {
    const { executor, seen } = makeExecutor({ exitCode: 0 })
    await executor({ websiteSlug: 'dash', script: 'websites/dash/run.sh' }, { signal })
    const env = seen[0].ctx.env
    expect(env.CRAFT_WORKSPACE_PATH).toBe('/tmp/ws')
    expect(env.CRAFT_WEBSITE_SLUG).toBe('dash')
    expect(env.CRAFT_WEBSITE_DIR).toBe(join('/tmp/ws', 'websites', 'dash'))
    expect(env.CRAFT_WEBSITE_DATA_DIR).toBe(join('/tmp/ws', 'websites', 'dash', 'data'))
  ...
  test('omits runtime/args when not pinned', async () => {
    const { executor, seen } = makeExecutor({ exitCode: 0 })
    await executor({ websiteSlug: 'dash', script: 'websites/dash/run.ts' }, { signal })
    expect(seen[0].action).toEqual({ type: 'script', script: 'websites/dash/run.ts' })
  })
  ...
  test('surfaces a non-zero exit as a resolved outcome (not a throw)', async () => {
    ...
    const out = await executor({ websiteSlug: 'dash', script: 'websites/dash/run.sh' }, { signal })
  ...
  test('throws when the run is blocked (path escape / missing runtime)', async () => {
    ...
    await expect(
      executor({ websiteSlug: 'dash', script: '../evil.sh' }, { signal }),
