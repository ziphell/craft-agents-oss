/**
 * Websites MCP executor: source resolution, usability gating, api-vs-mcp grant
...
import { createWebsitesMcpExecutor, MCP_ACTION_BODY_MAX_CHARS } from '../mcp-executor'
...
function makeExecutor(opts: {
  ...
  const executor = createWebsitesMcpExecutor({
...
describe('createWebsitesMcpExecutor', () => {
  ...
  test('throws the stable source-auth-required error for an unauthenticated source', async () => {
    ...
    // The prefix is part of the website authoring contract (docs/websites.md) —
    // websites match on it to disable buttons and defer to the reconnect banner.
    await expect(
      executor({ sourceSlug: 'craft', toolName: 't', args: {} }, { signal }),
