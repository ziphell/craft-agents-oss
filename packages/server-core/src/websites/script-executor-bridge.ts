/**
 * Websites script action executor.
 *
 * Backs WebsiteActionBroker.executors.executeScript for script-kind grants: runs
 * the grant's pinned workspace-relative script through the SAME hardened runner
 * website refreshes use (executeScriptAction) — argv/no-shell, workspace-contained
 ...
 * exit (the website still wants stdout/stderr); throw only when the script could
 ...
 * runner overwrite the website's scheduled-refresh completion marker
 * (recordWebsiteRefresh); an on-demand run a user clicks must not masquerade as a
 * refresh. CRAFT_WEBSITE_* env is still injected via buildBaseScriptEnv so the
...
import type { WebsiteScriptRuntime } from '@craft-agent/core'
import type { Logger } from '@craft-agent/server-core/runtime'

export interface WebsitesScriptExecutorDeps {
...
export function createWebsitesScriptExecutor(deps: WebsitesScriptExecutorDeps) {
  ...
  return async (
    ...
    if (result.blocked) {
      ...
      deps.log.warn(`[websites] script blocked for ${invocation.websiteSlug}: ${result.stderr}`)
