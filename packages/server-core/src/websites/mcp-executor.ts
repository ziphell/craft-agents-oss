/**
 * Websites MCP action executor.
 *
 * Backs WebsiteActionBroker.executors.executeMcp for mcp-kind grants: resolves
 ...
 *
 * Thrown errors become `{ok: false, error}` action results in the website;
...
import { assertWebsiteSourceUsable } from './source-gate'

/** Cap on tool-result text returned to a website (mirrors the api-kind cap in rpc/websites.ts). */
export const MCP_ACTION_BODY_MAX_CHARS = 512 * 1024

export interface WebsitesMcpExecutorDeps {
  workspaceRootPath: string
  /** Workspace-scoped pool, shared across all websites of the workspace */
  pool: McpClientPool
  log: Logger
...
export function createWebsitesMcpExecutor(deps: WebsitesMcpExecutorDeps) {
  ...
  return async (
    ...
    // cycle; websites have none, so heal here before the build — otherwise an
    ...
    if (!config) {
      ...
      if (buildError) {
        ...
        // error over generic connect prose so the website and the reconnect
