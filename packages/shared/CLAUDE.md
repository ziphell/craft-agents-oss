...
- `src/websites/` — workspace-scoped mini sites (`websites/{slug}/website.json`); storage, refresh hook, action bridge
...
- **Websites** (`src/websites/`): workspace single-file sites — one HTML document at one address (several screens inside are fine; no routes, no second file), at `websites/{slug}/` — `website.json` (config, validated on write; a `refresh.cron` is checked with croner — the same engine the scheduler matches with — and must parse, actually fire, and respect the 5-minute floor `WEBSITE_REFRESH_MIN_INTERVAL_MS`), `index.html` (content; sha256 `contentDigest`), `data/` (script-private `store.sqlite` + atomically-written `snapshot.json`, the ONLY cross-process data contract). `website.json` is deliberately the LAST write of a refresh run — the config watcher reads it as "this run is complete". `index.html` is a watcher trigger too: the website's files are the truth, so an out-of-band edit is realigned by `syncWebsiteContentDigest` (shared/websites/storage.ts) before the list is reloaded, and the host re-renders the poster only when the digest actually moved (`onWebsitesContentChange` → `SessionManager.enqueueWebsiteThumbnail`). Nothing else configures that digest: grants, render leases and the poster are all keyed to it, and `WebsiteActionBroker.validate` re-checks it against the live config on every request — which is what makes "edit the file" safe rather than a hole. Scheduled refresh = `WebsiteConfig.refresh` → synthetic cron matcher (`websites/refresh.ts`) merged into `AutomationSystem.getMatchersForEvent('SchedulerTick')`; rebuild via `reloadWebsiteRefreshMatchers()` (wired to the watcher in SessionManager). `websites/data-store.ts` is **Bun-only** (`bun:sqlite`) — subpath `@craft-agent/shared/websites/data-store` only, never re-export from the websites barrel, never import from Electron main.
- **Website data writes from hosts** (`websites/data-write.ts`, Node-safe, ON the websites barrel): `writeWebsiteData`/`applyWebsiteDataPatch` apply a `WebsiteDataPatch` (kv set/delete + series append/prune) by generating a **self-contained Bun one-shot** (argv spawn via `resolveScriptRuntime`) — the SQLite handle never lives in the host process, and it works under both Node (Electron main) and Bun (headless) hosts, dev and packaged. The generated script duplicates the data-store schema/SQL **on purpose** (zero module resolution from a temp dir); parity is pinned by `data-write.test.ts` — change schema/snapshot shape in BOTH files + test or not at all. `writeWebsiteData` stamps `recordWebsiteRefresh` LAST (watcher marker). Both engines set `PRAGMA busy_timeout = 5000` so concurrent refresh-script/tool writes wait instead of failing with SQLITE_BUSY, and both enforce the store growth caps (`WEBSITE_DATA_MAX_KV_KEYS` / `WEBSITE_DATA_MAX_SERIES`, data-store-constants.ts) post-write inside the write transaction — a violating write rolls back whole.
- **Websites session tools** (`list_websites`/`get_website` Explore-safe read-only; `create_website`/`update_website`/`write_website_data`/`delete_website` blocked in Explore — all derived from `safeMode` metadata in session-tools-core `tool-defs.ts`): handlers live in session-tools-core (`handlers/websites.ts`) and call the **grouped** `ctx.websites` callbacks (`WebsiteToolCallbacks` — one registry field, unlike the flat session-management Fns) bound by `session-self-management-bindings.ts` and implemented in server-core `websites/tool-callbacks.ts` (wired per-session in SessionManager next to `createTaskFn`). Delete goes through `deleteWebsiteWithUnpublish` (publisher.ts), shared verbatim with the `websites:delete` RPC — never re-implement unpublish-before-delete. Agent-facing authoring guide: `apps/electron/resources/docs/websites.md` (auto-synced to `~/.craft-agent/docs/`); system-prompt section in `prompts/system.ts`.
- **Website thumbnails** (cached poster): `WebsiteConfig.thumbnail` is a managed pointer (`{digest,capturedAt,w,h}`, excluded from `updateWebsite`, written only by `recordWebsiteThumbnail`) to `websites/{slug}/thumbnail.jpg`. `isThumbnailFresh(config)` = `thumbnail.digest === contentDigest` (single source of truth, used by the `websites:getThumbnail` RPC and the capturer); a content change leaves the pointer but makes it stale (data-only refreshes do NOT invalidate — v1). Generation is Electron-main-only (`apps/electron` `website-thumbnailer.ts`), injected via `SessionManager.setWebsiteThumbnailer`; `enqueueWebsiteThumbnail` no-ops headless. Never re-export capture code from shared — shared only owns the storage/freshness helpers.
- **Website action bridge** (`websites/action-bridge.ts`): website JS never calls sources — `WebsiteActionBroker` validates render leases (nonce + content digest + replay cache) and persisted grants (content-digest-bound + expiring, mirroring the privilegedExecutionBroker commandHash pattern), executes via injected executors under AbortSignal, and audit-logs every decision to `~/.craft-agent/logs/website-actions.jsonl` (redacted via `utils/redaction.ts`). Endpoint/tool policy lives in the freestanding `agent/source-policy.ts` (`evaluateApiEndpointPolicy` / `evaluateMcpToolPolicy`) — `shouldPromptInAskMode` delegates to it; use it for any non-hook policy check instead of re-implementing. `sources/api-tools.ts:executeApiRequest` is the single fetch path for API sources (supports `signal`/`timeoutMs`; never logs header values), and `PoolClient.callTool` accepts `{ signal, timeoutMs }` forwarded to the MCP SDK.
- **Websites sharing** (`websites/share-bundle.ts` + `websites/publisher.ts`, flag `CRAFT_FEATURE_WEBSITES_SHARING` default **ON** since 2026-08-27 — Worker deployed live; set `=0` to opt out): publishes a sanitized bundle (allowlisted manifest + index.html + opt-in snapshot; grants/refresh/sqlite/paths excluded by construction) to the `workers/pages` Cloudflare Worker. The publication **admin token** is a 256-bit capability stored ONLY in the credential vault as `website_publish_token::{workspaceId}::{pageId}` — never in `website.json` (`WebsiteConfig.share` is a display pointer written via `setPageShareState`; `updateWebsite` treats `share` as managed). The flag gates publish/republish/password only — **unpublish always works** (RPC `websites:unpublish`; website delete auto-unpublishes best-effort). Websites with **usable** grants (`isWebsiteGrantUsable`: digest-bound + unexpired — the single usable/stale definition, shared with the render frame and `get_website`) require `viewOnlyAcknowledged` (public copy answers bridge actions with `public-actions-disabled`); a **script** grant refuses publish outright even when stale — the remedy is removal (`websites:revokeGrant`, surfaced in the Share dialog and the website's Approved-actions dialog), never bypass. Local dev endpoint override: `CRAFT_WEBSITES_SHARE_API_URL`. Worker code/tests/deploy: `workers/pages/` (bun-testable end-to-end without Cloudflare).
