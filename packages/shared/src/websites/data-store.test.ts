/**
 * Tests for the Bun-only SQLite website data store (bun:sqlite — these tests
...
import { openWebsiteDataStore, WebsiteDataStore, WEBSITE_DATA_MAX_KV_KEYS, WEBSITE_DATA_MAX_SERIES } from './data-store.ts';
import { getWebsiteSnapshotPath, getWebsiteStorePath } from './storage.ts';

describe('websites/data-store', () => {
  let workspaceDir: string;
  let store: WebsiteDataStore;

  beforeEach(() => {
    workspaceDir = mkdtempSync(join(tmpdir(), 'website-data-store-test-'));
    store = openWebsiteDataStore(workspaceDir, 'dash');
  });
  ...
  describe('growth caps', () => {
    it('rejects the kv write that would exceed the key cap and rolls it back', () => {
      for (let i = 0; i < WEBSITE_DATA_MAX_KV_KEYS; i++) {
        store.kvSet(`k${i}`, i);
      }
      ...
      expect(Object.keys(store.kvEntries()).length).toBe(WEBSITE_DATA_MAX_KV_KEYS);
    ...
    it('rejects the series write that would exceed the series cap and rolls it back', () => {
      for (let i = 0; i < WEBSITE_DATA_MAX_SERIES; i++) {
      ...
      expect(store.seriesNames().length).toBe(WEBSITE_DATA_MAX_SERIES);
  ...
  describe('snapshot export', () => {
    it('writes snapshot.json atomically with kv + capped series', () => {
      ...
      const snapshotPath = getWebsiteSnapshotPath(workspaceDir, 'dash');
    ...
    it('persists across store reopen (sqlite file is the working store)', () => {
      ...
      expect(existsSync(getWebsiteStorePath(workspaceDir, 'dash'))).toBe(true);
    });
  });
});
