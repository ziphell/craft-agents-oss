/**
 * Website Data Write — Node-safe patch application
 *
 * Applies a `WebsiteDataPatch` (kv upserts/deletes + timeseries appends/prunes)
 * to a website's SQLite store and regenerates data/snapshot.json — WITHOUT ever
 ...
 * with websites/data-store.ts is pinned by data-write.test.ts — if you change
...
import type { WebsiteConfig, WebsiteDataSnapshot } from '@craft-agent/core';
...
const log = createLogger('website-data-write');
...
export const WEBSITE_DATA_PATCH_MAX_BYTES = 1024 * 1024;
 ...
 */
export interface WebsiteDataPatch {
...
export interface WebsiteDataWriteResult {
  websiteSlug: string;
...
export function validateWebsiteDataPatch(patch: WebsiteDataPatch): string | null {
  const hasOps =
  ...
  if (Buffer.byteLength(serialized, 'utf-8') > WEBSITE_DATA_PATCH_MAX_BYTES) {
    return `Patch exceeds ${WEBSITE_DATA_PATCH_MAX_BYTES} bytes when serialized`;
  }
  return null;
}
 ...
 * websites/data-store.ts (see the parity test).
 */
export function buildWebsiteDataWriterScript(input: {
  dbPath: string;
  snapshotPath: string;
  patch: WebsiteDataPatch;
  ...
  return `// Generated by @craft-agent/shared/websites/data-write — self-contained on purpose.
...
// a timeout set first. (Kept in lockstep with websites/data-store.ts.)
...
const applyAll = db.transaction(() => {
  ...
  // back whole. (Kept in lockstep with WebsiteDataStore's assertKvCap/assertSeriesCap.)
  const kvCount = db.query('SELECT COUNT(*) AS n FROM kv').get().n;
  if (kvCount > INPUT.maxKvKeys) {
    throw new Error('Website data store limit exceeded: ' + kvCount + ' kv keys > max ' + INPUT.maxKvKeys + '. Delete unused keys instead of growing the store.');
  }
  const distinctSeries = db.query('SELECT COUNT(DISTINCT series) AS n FROM timeseries').get().n;
  if (distinctSeries > INPUT.maxSeries) {
    throw new Error('Website data store limit exceeded: ' + distinctSeries + ' series > max ' + INPUT.maxSeries + '. Reuse series names and prune old ones instead of growing the store.');
  }
});
applyAll();

// Snapshot export — mirrors WebsiteDataStore.exportSnapshot exactly, including the
...
/**
 * Apply a data patch to a website's store and regenerate its snapshot.
 *
 * Pure data operation — does NOT touch website.json. Callers that want observers
 * to react (watcher → `websites:changed`) should use writeWebsiteData(), which
 * stamps the refresh marker as the LAST write, per the locked design.
 */
export async function applyWebsiteDataPatch(
  workspaceRootPath: string,
  websiteSlug: string,
  patch: WebsiteDataPatch,
  options?: { maxPointsPerSeries?: number },
): Promise<Omit<WebsiteDataWriteResult, 'websiteSlug'>> {
  const validationError = validateWebsiteDataPatch(patch);
  if (validationError) {
    throw new Error(`Invalid website data patch: ${validationError}`);
  }

  const runtime = resolveScriptRuntime('bun');
  const script = buildWebsiteDataWriterScript({
    dbPath: getWebsiteStorePath(workspaceRootPath, websiteSlug),
    ...
    maxKvKeys: WEBSITE_DATA_MAX_KV_KEYS,
    maxSeries: WEBSITE_DATA_MAX_SERIES,
  });

  const startTime = Date.now();
  const tempDir = mkdtempSync(join(tmpdir(), 'craft-website-data-'));
  ...
  try {
    ...
    const output = await new Promise<string>((resolve, reject) => {
      ...
      child.on('error', (error) => {
        ...
        reject(new Error(`Website data writer failed to spawn: ${error.message}`));
      });
      child.on('close', (code) => {
        clearTimeout(termTimer);
        clearTimeout(killTimer);
        if (timedOut) {
          reject(new Error(`Website data write timed out after ${WRITE_TIMEOUT_MS}ms`));
        } else if (code !== 0) {
          reject(new Error(`Website data write failed (exit ${code}): ${stderr.trim().slice(0, 2000) || 'no stderr'}`));
        } else {
          resolve(stdout);
        }
      });
    ...
    throw new Error('Website data writer produced no result line');
  } finally {
...
/**
 * Apply a data patch AND stamp the refresh marker on website.json (LAST write),
 * so the config watcher broadcasts `websites:changed` and open renders receive
 ...
 */
export async function writeWebsiteData(
  workspaceRootPath: string,
  websiteSlug: string,
  patch: WebsiteDataPatch,
): Promise<{ result: WebsiteDataWriteResult; config: WebsiteConfig }> {
  const existing = loadWebsiteConfig(workspaceRootPath, websiteSlug);
  if (!existing) {
    throw new Error(`Website not found: ${websiteSlug}`);
  }

  const applied = await applyWebsiteDataPatch(workspaceRootPath, websiteSlug, patch);
  const config = recordWebsiteRefresh(workspaceRootPath, websiteSlug, {
    at: Date.now(),
    ok: true,
    durationMs: applied.durationMs,
  });
  log.debug(`[writeWebsiteData] ${websiteSlug}: kv=${applied.kvCount} series=${applied.seriesCount} in ${applied.durationMs}ms`);
  return { result: { websiteSlug, ...applied }, config };
}

/** Re-export for snapshot readers that want the shared type. */
export type { WebsiteDataSnapshot };
