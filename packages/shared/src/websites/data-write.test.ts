...
import { createWebsite, getWebsiteSnapshotPath, getWebsiteStorePath, loadWebsiteConfig } from './storage.ts';
import {
...
import { WebsiteDataStore, WEBSITE_DATA_MAX_KV_KEYS, WEBSITE_DATA_MAX_SERIES } from './data-store.ts';
import type { WebsiteDataSnapshot } from '@craft-agent/core';
...
describe('website data write (spawned Bun one-shot)', () => {
  ...
  function readSnapshot(): WebsiteDataSnapshot {
    return JSON.parse(readFileSync(getWebsiteSnapshotPath(workspace, slug), 'utf-8'));
  }

  it('applies kv + series writes and exports a snapshot the host can read', async () => {
    const result = await applyWebsiteDataPatch(workspace, slug, {
  ...
  it('is parity-compatible with the bun:sqlite data store in BOTH directions', async () => {
    ...
    const store = new WebsiteDataStore(getWebsiteStorePath(workspace, slug));
    ...
    await applyWebsiteDataPatch(workspace, slug, {
      set: { fromWriter: 1 },
    ...
    const verify = new WebsiteDataStore(getWebsiteStorePath(workspace, slug));
  ...
  it('writeWebsiteData stamps the refresh marker on website.json LAST', async () => {
    const before = loadWebsiteConfig(workspace, slug);
    const { result, config } = await writeWebsiteData(workspace, slug, { set: { stamped: true } });

    expect(result.websiteSlug).toBe(slug);
    expect(config.lastRefresh?.ok).toBe(true);
    expect(config.updatedAt).toBeGreaterThanOrEqual(before!.updatedAt);
    // Marker written after the snapshot: observers reacting to website.json see complete data.
    expect(readSnapshot().kv.stamped).toBe(true);
    expect(loadWebsiteConfig(workspace, slug)?.lastRefresh?.at).toBe(config.lastRefresh!.at);
  });
  ...
  it('concurrent writers never corrupt the snapshot and lose no writes (unique temp files)', async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: N }, (_, i) =>
        applyWebsiteDataPatch(workspace, website, {
          set: { [`k${i}`]: i },
    ...
    const snapshot = JSON.parse(readFileSync(getWebsiteSnapshotPath(workspace, website), 'utf-8')) as WebsiteDataSnapshot;
    expect(snapshot.version).toBe(1);

    // Every write committed: re-export after all writers finished sees all N.
    const store = new WebsiteDataStore(getWebsiteStorePath(workspace, website));
  ...
  it('enforces the kv/series growth caps transactionally (violating patch rolls back whole)', async () => {
    ...
    for (let i = 0; i < WEBSITE_DATA_MAX_KV_KEYS; i++) kvOverflow[`k${i}`] = i; // 2 seeds + 1000 > cap
    await expect(applyWebsiteDataPatch(workspace, website, { set: kvOverflow }))
      .rejects.toThrow(/kv keys > max/);

    const seriesOverflow: Record<string, Array<{ t: number; v: number }>> = {};
    for (let i = 0; i < WEBSITE_DATA_MAX_SERIES; i++) seriesOverflow[`s${i}`] = [{ t: 1, v: i }]; // 1 seed + 100 > cap
    await expect(applyWebsiteDataPatch(workspace, website, { appendSeries: seriesOverflow }))
  ...
  it('rejects unknown websites and invalid patches without spawning', async () => {
    await expect(writeWebsiteData(workspace, 'nope', { set: { a: 1 } })).rejects.toThrow('Website not found');
    await expect(applyWebsiteDataPatch(workspace, slug, {})).rejects.toThrow('no operations');
    await expect(applyWebsiteDataPatch(workspace, slug, { appendSeries: { m: [{ v: Number.NaN }] } }))
      .rejects.toThrow('non-finite');
    await expect(applyWebsiteDataPatch(workspace, slug, { pruneSeries: { m: 1.5 } }))
      .rejects.toThrow('integer timestamp');

    expect(validateWebsiteDataPatch({ set: { big: 'x'.repeat(WEBSITE_DATA_PATCH_MAX_BYTES) } }))
      .toContain('exceeds');
  });
});
