/**
 * Website Data Store — BUN-ONLY (bun:sqlite)
 *
 * Working store for website refresh scripts: a small key-value + timeseries
 * layer over SQLite at websites/{slug}/data/store.sqlite.
 *
 ...
 *   that is why it is exported solely as the '@craft-agent/shared/websites/data-store'
 *   subpath and deliberately NOT re-exported from '@craft-agent/shared/websites'.
 ...
 *   only the snapshot, and website.json is the completion marker they watch.
 *
 ...
 *   store.close();
 *   // executor then records the run on website.json → watcher → websites:changed
 */

import { Database } from 'bun:sqlite';
import { dirname, join } from 'path';
import { mkdirSync } from 'fs';
import type { WebsiteDataSnapshot, WebsiteSeriesPoint } from '@craft-agent/core';
import { atomicWriteFileSync } from '../utils/files.ts';
import { getWebsiteSnapshotPath, getWebsiteStorePath, WEBSITE_SNAPSHOT_FILENAME } from './storage.ts';
import {
...
export { DEFAULT_SNAPSHOT_MAX_POINTS_PER_SERIES, WEBSITE_DATA_MAX_KV_KEYS, WEBSITE_DATA_MAX_SERIES };
...
export class WebsiteDataStore {
  private readonly db: Database;
  private readonly snapshotPath: string;

  constructor(dbPath: string, snapshotPath?: string) {
    mkdirSync(dirname(dbPath), { recursive: true });
    this.snapshotPath = snapshotPath ?? join(dirname(dbPath), WEBSITE_SNAPSHOT_FILENAME);
    ...
    // (refresh script vs. agent write_website_data).
  ...
  /** @throws when the kv table exceeds WEBSITE_DATA_MAX_KV_KEYS */
  private assertKvCap(): void {
    const row = this.db.query<{ n: number }, []>('SELECT COUNT(*) AS n FROM kv').get();
    if ((row?.n ?? 0) > WEBSITE_DATA_MAX_KV_KEYS) {
      throw new Error(
        `Website data store limit exceeded: ${row!.n} kv keys > max ${WEBSITE_DATA_MAX_KV_KEYS}. Delete unused keys (kvDelete) instead of growing the store.`,
      );
    }
  }

  /** @throws when distinct series exceed WEBSITE_DATA_MAX_SERIES */
  private assertSeriesCap(): void {
    const row = this.db.query<{ n: number }, []>('SELECT COUNT(DISTINCT series) AS n FROM timeseries').get();
    if ((row?.n ?? 0) > WEBSITE_DATA_MAX_SERIES) {
      throw new Error(
        `Website data store limit exceeded: ${row!.n} series > max ${WEBSITE_DATA_MAX_SERIES}. Reuse series names; prune old ones (seriesPrune) instead of growing the store.`,
      );
    }
  }
  ...
  /** Set a key to any JSON-serializable value. @throws over WEBSITE_DATA_MAX_KV_KEYS (write rolled back) */
   ...
   * @throws over WEBSITE_DATA_MAX_SERIES (write rolled back)
   */
  ...
  /** Append many datapoints in one transaction. @throws over WEBSITE_DATA_MAX_SERIES (batch rolled back) */
  ...
  seriesRange(series: string, options: SeriesRangeOptions = {}): WebsiteSeriesPoint[] {
  ...
  seriesLatest(series: string): WebsiteSeriesPoint | undefined {
    return (
      this.db
   ...
   */
  exportSnapshot(options: ExportSnapshotOptions = {}): WebsiteDataSnapshot {
    const maxPoints = options.maxPointsPerSeries ?? DEFAULT_SNAPSHOT_MAX_POINTS_PER_SERIES;
    const series: Record<string, WebsiteSeriesPoint[]> = {};
    let kv: WebsiteDataSnapshot['kv'] = {};
    ...
    const snapshot: WebsiteDataSnapshot = {
      version: 1,
...
/**
 * Open (creating if needed) the data store for a website.
 * Path layout comes from websites/storage.ts so scripts and host agree.
 */
export function openWebsiteDataStore(workspaceRootPath: string, websiteSlug: string): WebsiteDataStore {
  return new WebsiteDataStore(
    getWebsiteStorePath(workspaceRootPath, websiteSlug),
    getWebsiteSnapshotPath(workspaceRootPath, websiteSlug),
  );
}
