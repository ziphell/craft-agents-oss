/**
 * Website Action Bridge — mediated source actions for Websites
 *
 * Website JS never talks to sources. It posts a request up to the host, and this
 ...
 *                   mounts a website; carries a nonce the website must echo on
 *                   every request (frame identity without a trustable
 *                   Origin). Bound to the website's content digest — content
 *                   changes end the lease.
 *   grant         — persisted in website.json, user-approved, content-digest
 ...
 * appended to a durable JSONL audit log (~/.craft-agent/logs/website-actions.jsonl),
...
const log = createLogger('website-action-broker');

/** Default render-lease lifetime; re-mounting a website issues a fresh lease */
export const DEFAULT_WEBSITE_LEASE_TTL_MS = 12 * 60 * 60 * 1000;
/** Default per-action timeout */
export const DEFAULT_WEBSITE_ACTION_TIMEOUT_MS = 30_000;
/** Replay-cache cap per lease — beyond this the lease must be re-issued */
const MAX_SEEN_REQUEST_IDS_PER_LEASE = 5_000;
/**
 * Host-side per-lease budget. The renderer runs its own WebsiteActionRateLimiter
 ...
 * Matching the renderer's budget means a well-behaved website never hits this.
 */
export const WEBSITE_ACTION_MAX_IN_FLIGHT_PER_LEASE = 5;
export const WEBSITE_ACTION_MAX_STARTS_PER_MINUTE_PER_LEASE = 30;
const WEBSITE_ACTION_RATE_WINDOW_MS = 60_000;
/**
...
export type WebsiteActionValidationErrorCode =
  | 'lease-not-found'
  | 'lease-website-mismatch'
  | 'lease-expired'
  | 'nonce-mismatch'
  | 'replay'
  | 'replay-cache-full'
...
type ValidationOutcome =
  | { ok: true; grant: WebsiteActionGrant }
  | { ok: false; code: WebsiteActionValidationErrorCode; reason: string };
 ...
 */
export interface WebsiteActionExecutors {
  /** Execute an api-kind invocation against an API source */
  executeApi?: (
    invocation: { sourceSlug: string; method: WebsiteActionHttpMethod; path: string; params?: Record<string, unknown> },
   ...
   * host. Resolves with the process outcome even on a non-zero exit (the website
   * still wants stdout/stderr); throws only when the script could not run at
   * all (path escape, missing runtime, spawn failure).
   */
  executeScript?: (
    invocation: { websiteSlug: string; script: string; runtime?: WebsiteScriptRuntime; args?: string[] },
    options: { signal: AbortSignal },
  ) => Promise<{ exitCode: number | null; stdout: string; stderr: string }>;
}

export interface WebsiteActionBrokerOptions {
  executors: WebsiteActionExecutors;
  /** Audit log path (default: {CONFIG_DIR}/logs/website-actions.jsonl) */
  auditLogPath?: string;
...
export class WebsiteActionBroker {
  private readonly executors: WebsiteActionExecutors;
  ...
  private readonly leases = new Map<string, WebsiteRenderLease>();
  ...
  constructor(options: WebsiteActionBrokerOptions) {
    this.executors = options.executors;
    this.auditLogPath = options.auditLogPath ?? join(CONFIG_DIR, 'logs', 'website-actions.jsonl');
    this.leaseTtlMs = options.leaseTtlMs ?? DEFAULT_WEBSITE_LEASE_TTL_MS;
    this.actionTimeoutMs = options.actionTimeoutMs ?? DEFAULT_WEBSITE_ACTION_TIMEOUT_MS;
  ...
  /**
   * Issue a render lease for one mount of a website. The returned nonce travels
   * into the iframe and must be echoed on every action request.
   */
  createLease(input: CreateLeaseInput): WebsiteRenderLease {
    this.pruneExpiredLeases();

    if (this.leases.size >= MAX_LIVE_LEASES) {
      let oldest: WebsiteRenderLease | undefined;
      ...
      if (oldest) {
        ...
        void this.appendAudit({
          event: 'website_lease_evicted',
          websiteSlug: oldest.websiteSlug,
    ...
    const now = this.now();
    const lease: WebsiteRenderLease = {
      leaseId: randomUUID(),
    ...
    void this.appendAudit({
      event: 'website_lease_created',
      websiteSlug: lease.websiteSlug,
  ...
  /** Drop a lease (website unmounted). Idempotent. */
  releaseLease(leaseId: string): void {
    const lease = this.leases.get(leaseId);
    if (!lease) return;
    this.dropLease(leaseId);
    void this.appendAudit({ event: 'website_lease_released', websiteSlug: lease.websiteSlug, leaseId });
  }
  ...
  private rateLimitRejection(leaseId: string): { code: 'rate-limited'; reason: string } | null {
    if ((this.inFlightByLease.get(leaseId) ?? 0) >= WEBSITE_ACTION_MAX_IN_FLIGHT_PER_LEASE) {
      return {
        code: 'rate-limited',
        reason: `Too many actions in flight for this render (max ${WEBSITE_ACTION_MAX_IN_FLIGHT_PER_LEASE})`,
      };
    }
    const now = this.now();
    const starts = (this.startTimesByLease.get(leaseId) ?? []).filter(
      (t) => now - t < WEBSITE_ACTION_RATE_WINDOW_MS,
    );
    this.startTimesByLease.set(leaseId, starts);
    if (starts.length >= WEBSITE_ACTION_MAX_STARTS_PER_MINUTE_PER_LEASE) {
      return {
        code: 'rate-limited',
        reason: `Too many actions this minute for this render (max ${WEBSITE_ACTION_MAX_STARTS_PER_MINUTE_PER_LEASE}/minute)`,
      };
    }
    return null;
  }
  ...
  private validate(website: WebsiteConfig, request: WebsiteActionRequest): ValidationOutcome {
    const now = this.now();

    const lease = this.leases.get(request.leaseId);
    if (!lease) {
      return { ok: false, code: 'lease-not-found', reason: 'No render lease for this request — re-mount the website' };
    }
    if (lease.websiteSlug !== request.websiteSlug || website.slug !== request.websiteSlug) {
      return { ok: false, code: 'lease-website-mismatch', reason: 'Lease belongs to a different website' };
    }
    if (now > lease.expiresAt) {
      this.dropLease(request.leaseId);
      return { ok: false, code: 'lease-expired', reason: 'Render lease expired — re-mount the website' };
    }
    if (lease.nonce !== request.nonce) {
      return { ok: false, code: 'nonce-mismatch', reason: 'Request nonce does not match the render lease' };
    }

    if (!website.contentDigest) {
      return { ok: false, code: 'content-missing', reason: 'Website has no content digest' };
    }
    if (lease.contentDigest !== website.contentDigest) {
      return { ok: false, code: 'content-changed', reason: 'Website content changed since this render — re-mount the website' };
    }
    ...
    if (seen && seen.size >= MAX_SEEN_REQUEST_IDS_PER_LEASE) {
      return { ok: false, code: 'replay-cache-full', reason: 'Lease exhausted its request budget — re-mount the website' };
    }

    const grant = website.grants?.find((g) => g.id === request.grantId);
    if (!grant) {
      return { ok: false, code: 'grant-not-found', reason: `No grant ${request.grantId} on this website` };
    }
    if (grant.contentDigest !== website.contentDigest) {
      return { ok: false, code: 'grant-stale', reason: 'Grant was approved for older website content — re-approval required' };
    }
  ...
  private invocationMismatch(grant: WebsiteActionGrant, invocation: WebsiteActionInvocation): ValidationOutcome | null {
  ...
  /**
   * Validate and execute one website action request against the current
   * website.json state. Never throws — failures come back as { ok: false }.
   */
  async executeAction(website: WebsiteConfig, request: WebsiteActionRequest): Promise<WebsiteActionResult> {
    const startTime = this.now();
    const invocationSummary = this.summarizeInvocation(request.invocation);

    const validation = this.validate(website, request);
    if (!validation.ok) {
      void this.appendAudit({
        event: 'website_action_rejected',
      ...
      if (grant.action.kind === 'api' && request.invocation.kind === 'api') {
        ...
        if (!this.executors.executeScript) {
          ...
          result = {
            requestId: request.requestId,
            ok,
            // The website sees stdout/stderr/exit even on failure — a script that
  ...
  cancelAction(requestId: string): boolean {
    ...
    controller.abort();
    void this.appendAudit({ event: 'website_action_cancelled', requestId });
    return true;
  }
  ...
  private unavailableResult(request: WebsiteActionRequest, startTime: number, reason: string): WebsiteActionResult {
    return {
      requestId: request.requestId,
      ok: false,
  ...
  private summarizeInvocation(invocation: WebsiteActionInvocation): Record<string, unknown> {
  ...
  private async appendAudit(payload: Record<string, unknown>): Promise<void> {
    try {
      ...
      log.warn(`[WebsiteActionBroker] Failed to write audit log: ${error}`);
    }
  }
}
